...
const simpleAccountFactoryContract = new web3.eth.Contract(
...
async function getBalance() {
  console.log(
    `Alice ETH Balance ${web3.utils.fromWei(
      await web3.eth.getBalance(alicePublicKey)
    )}`
  );
  console.log(
    `Alice sender wallet ${walletOwner} ETH Balance ${web3.utils.fromWei(
      await web3.eth.getBalance(walletOwner)
    )}`
  );
  console.log(
    `Paymaster ETH Balance ${web3.utils.fromWei(
      await payMasterContract.methods.getDeposit().call()
    )}`
  );

  console.log(
    `Alice WETH Balance ${web3.utils.fromWei(
      await wethContract.methods.balanceOf(alicePublicKey).call()
    )}`
  );
  console.log(
    `Alice sender wallet ${walletOwner}  WETH Balance ${web3.utils.fromWei(
      await wethContract.methods.balanceOf(walletOwner).call()
    )}`
  );

  console.log(
    `Bob WETH Balance ${web3.utils.fromWei(
      await wethContract.methods.balanceOf(bobPublicKey).call()
    )}`
  );
  console.log(
    `Bob DAI Balance ${web3.utils.fromWei(
      await daiContract.methods.balanceOf(bobPublicKey).call()
    )}`
  );
}
...
async function initAddresses() {
  ...
  walletOwner = await simpleAccountFactoryContract.methods
    .getAddress(alicePublicKey, salt)
    .call();
}
...
async function composeInitCode() {
  const walletCreateABI = simpleAccountFactoryContract.methods
    .createAccount(alicePublicKey, salt)
    .encodeABI();
...
async function composePaymasterAndData(ops) {
  ...
  const hash = await payMasterContract.methods
    .getHash(ops, MOCK_VALID_UNTIL, MOCK_VALID_AFTER)
    .call();
...
async function fundContractsAndAddresses() {
  ...
  rawData = await entryPointContract.methods
    .depositTo(payMasterAddress)
    .encodeABI();
  await executeOnChainTransaction(
    "2",
    rawData,
...
async function executeHandleOps(initCode, callData, viaPaymaster) {
  ...
  walletOwner = await simpleAccountFactoryContract.methods
    .getAddress(alicePublicKey, salt)
    .call();
  const nonce = await entryPointContract.methods
    .getNonce(walletOwner, 0)
    .call();
  ...
  if (callData != "0x")
    callData = await walletContract.methods
      .execute(WETH, 0, callData)
      .encodeABI();
  ...
  if (viaPaymaster) ops.paymasterAndData = await composePaymasterAndData(ops);
  ...
  const enc = defaultAbiCoder.encode(
    ...
    [userOpHash, entryPointAddress, 1]
  );
  ...
  await web3.eth.sendSignedTransaction(
    ...
    function (error, hash) {
      if (!error) {
        console.log("Handleops Success --> ", hash);
      } else {
